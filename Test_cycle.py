# no pruned, part of matching point to laplacian
import argparse
import numpy as np
...
class cycle_tester():
    ...
    def official_RR(self, dataset):
        ...
        temp_precision, temp_recall, c_flag, c_error = evaluate_registration(n_fragments, est_traj, est_pairs, gt_pairs,
                                                                             gt_traj, gt_traj_cov, err2=self.tau_2,
    ...
    def construct_LSW(self, dataset):
        ...
        overlap_score = np.load(f'{self.cfg.save_dir}/predict_overlap/{dataset.name}/connectMatrix.npy')
        n, _ = scoremat.shape
        # keep symmetry
        for i in range(n):
            scoremat[i, i] = 0
            ...
            for j in range(i + 1, n):
                ...
                overlap_score[j, i] = overlap_score[i, j]
        # conduct top-k mask
        mask = np.zeros([n, n])
        for i in range(n):
    ...
    def laplacian_check(self, des0, des1):
        f0 = torch.transpose(des0, 0, 1)[None, :, :]
        f1 = torch.transpose(des1, 0, 1)[None, :, :]
        # f0 = torch.from_numpy(np.transpose(des0.astype(np.float32))[None, :, :]).cuda()
        # f1 = torch.from_numpy(np.transpose(des1.astype(np.float32))[None, :, :]).cuda()
        temp = torch.matmul(f0.permute(0, 2, 1), f1)
        #KNN_adj = knn_module.KNN(des0.shape[0])
        #adj, _ = KNN_adj(f0, f1)
        adj = torch.squeeze(torch.abs(temp))
        ind = torch.argmax(adj, dim=1)
        adj = adj - torch.diag_embed(torch.diag(adj))   # diag->0
        # 先求度矩阵
        R = torch.sum(adj, axis=1)
        degreeMatrix = torch.diag(R)
        laplacian_matrix = degreeMatrix - adj
        L = eigs(laplacian_matrix.cpu().numpy(), k=2, which='SR', maxiter=20, tol=0.1)[0][1].real
        return L#*des0.shape[0]
    ...
    def confidence(self, dataset, name, setname):
        ...
        ratio = np.zeros((n, n))
        c = np.zeros((n, n))
        connectMatrix = np.zeros((n, n))
        connectMatrix_0 = np.zeros((n, n))
        connectMatrix_1 = np.zeros((n, n))
        corr = {}
        keys0 = []
        keys1 = []
        #t1 = time.time()
        for i in range(n):
            for j in range(n):
                if i >= j:
                    continue
                t0 = time.time()
                keys0 = dataset.get_kps(i)  # keypoints 5000*3
                keys1 = dataset.get_kps(j)  # keypoints
                des0 = self.get_des(dataset, i)     # feature 5000*32
                des1 = self.get_des(dataset, j)
                n_matches = self.match(des0, des1)  # matching point 1232*2

                corr[(i, j)] = n_matches
                corr[(j, i)] = n_matches    # corr is a symmetry matrix, n*n
                keys0 = keys0[n_matches[:, 0]]  # extract keypoints of correspondence point, 1232*3
                keys1 = keys1[n_matches[:, 1]]
                des0 = des0[n_matches[:, 0]]    # extract feature of correspondence point, 1232*32
                des1 = des1[n_matches[:, 1]]
                des0 = torch.from_numpy(des0).cuda()    # (n, 32), 1232*32
                des1 = torch.from_numpy(des1).cuda()
                # self laplacian
                connectity_0 = self.laplacian_check(des0, des0)
                connectity_1 = self.laplacian_check(des1, des1)
                connectMatrix[i][j] = connectity_0 + connectity_1
                connectMatrix[j][i] = connectity_0 + connectity_1
        # construct mask
        for i in range(n):# keep symmetry
            connectMatrix[i, i] = 0
            for j in range(i + 1, n):
                connectMatrix[j, i] = connectMatrix[i, j]
        mask = np.zeros([n, n])
        for i in range(n):
            score_scan = connectMatrix[i] # connectMatrix[i]
            argsort = np.argsort(-score_scan)[:args.topk]
            mask[i, argsort] = 1

        for i in range(n):
            for j in range(n):
                if i >= j or mask[i, j] < 1:
                    continue
                # compute tranformation confidence
                keys0 = dataset.get_kps(i)
                keys1 = dataset.get_kps(j)
                keys0 = keys0[corr[(i, j)][:, 0]]
                keys1 = keys1[corr[(i, j)][:, 1]]
                keys_0 = torch.from_numpy(keys0).cuda()
                keys_1 = torch.from_numpy(keys1).cuda()

                src_dist = torch.norm((keys_0[:, None, :] - keys_0[None, :, :]), dim=-1)
                tgt_dist = torch.norm((keys_1[:, None, :] - keys_1[None, :, :]), dim=-1)
                ...
                ratio[i][j] = score
                ratio[j][i] = score
        min_val = np.sort(connectMatrix, axis=1)[:, 1]
        max_val = np.max(connectMatrix, axis=1)
        connectMatrix_normed = (connectMatrix - min_val) / (max_val - min_val)
        ratio = ratio / np.sqrt(np.sum(ratio ** 2) / 2)
        ratio = ratio
        target_path = os.path.join(self.cfg.pre_path, setname, name)
        if not os.path.exists(target_path):
            os.makedirs(target_path)
        np.save(os.path.join(target_path, 'ratio.npy'), ratio)
        np.save(os.path.join(target_path, 'connectMatrix.npy'), connectMatrix_normed)
        return corr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sigma_d', default=0.15, type=float, help='')
    parser.add_argument('--sigma_f', default=0.05, type=float, help='')
