...
class p2preg():
    ...
    def ransac(self, keys0, keys1, match):
        ...
        source_pcd.points = open3d.utility.Vector3dVector(keys0)  # numpy to open3d.pointcloud
        ...
        Keys_m0 = keys0[match[:, 0]]
        Keys_m1 = keys1[match[:, 1]]
        trans = self.refiner.Refine_trans(Keys_m0, Keys_m1, trans, np.ones(match.shape[0]),
                                          inlinerdist=self.inlierd * 2.0)
    ...
    def run(self, dataset, id0, id1, match):
...
class fcgf(p2preg):
    def get_des(self, dataset, sid):
        desdir = f'data/{dataset.name}/fcgf_desc'
        des = np.load(f'{desdir}/{sid}.npy')
        return des

    def already_exists(self, dataset, id0, id1):
        fn = f'./pre/pairwise_registration/fcgf/{dataset.name}/{id0}-{id1}.npz'
        if os.path.exists(fn):
            T = np.load(fn)
            T, ir, n_matches = T['trans'], T['ir'], T['n_matches']
            return True, T, ir, n_matches
    ...
    def save(self, dataset, id0, id1, T, ir, n_matches):
        savedir = f'./pre/pairwise_registration/fcgf/{dataset.name}/'
        make_non_exists_dir(savedir)
        np.savez(f'{savedir}/{id0}-{id1}.npz', trans=T, ir=ir, n_matches=n_matches)
