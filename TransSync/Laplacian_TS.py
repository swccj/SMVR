...
def rh_compute(iterid, Wh, Rh, locws, Ts, Tpre, rh):
    # print(Rh.shape)
    for pid in range(locws.shape[0]):
...
def pair2globalT_cycle(W_edges, Ts, iters):
    ...
    Wh = locws[:, -1, None]
    Rh = np.zeros(locws.shape[0])
    rh = np.empty(locws.shape[0])
    Vt = np.zeros((locws.shape[0], 1))
    vt = np.empty((locws.shape[0], 1))
    # cycle re-weighting
    Tpre = LaplacianT(locws, Rs, ts, N)
    for iterid in range(iters):
        for pid in range(locws.shape[0]):
            i, j = int(locws[pid, 0]), int(locws[pid, 1])
            Ti = Tpre[i]
            Tj = Tpre[j]
            ...
            rdiff = compute_R_diff(Tijfit[0:3, 0:3], Tijpre[0:3, 0:3])
            rh[pid] = rdiff
        vt[:, 0] = 0.01 * (rh - Rh) + 0.98 * Vt[:, iterid] + 0.01 * rh
        # vt[:, 0] = 0.01 * (rh - Rh) + 1*(0.98 * Vt[:, iterid] + 0.02 * rh)
        Rh = rh
        Vt = np.concatenate((Vt, vt), axis=1)
        W = np.exp(-1 * (Vt[:, 1:]))
        norm = np.linalg.norm(W, axis=1, keepdims=True)
        W = np.divide(W, norm)
        locws[:, -1] = np.sum(np.multiply(W, Wh), axis=1)  # [i,j,w] n*3, n= number of W_edges>0
        locws = keep_symmetry(locws, N)
        h = locws[:, -1, None]  # [w] n*1, n= number of W_edges>0
